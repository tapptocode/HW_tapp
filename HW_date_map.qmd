...
extract_month_name <- possibly(function(d_string) {
  ...
  if (!is.na(date)) {
    return(format(date,"%B")) ### If we don't get NA and instead get a date, this will reformat it into returning the month's name
  } else {
    return(NA) ### This is to give us an NA if ymd and dmy don't work
  }
})

month_names <- map(date_strings,extract_month_name) ### map will have our newly created function go through all of date_strings
